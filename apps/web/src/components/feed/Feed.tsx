...
export function Feed({
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-full snap-y snap-mandatory overflow-y-scroll overscroll-y-contain no-scrollbar"
    ...
    >
      {pullDistance > 0 && (
        ...
        >
          <svg width="28" height="28" viewBox="0 0 28 28" className="text-primary-500">
            <motion.circle
              cx="14"
              cy="14"
              r="10"
              ...
              strokeWidth="3"
              strokeDasharray="60"
              strokeDashoffset={60 - 60 * pullProgress}
              strokeLinecap="round"
            />
            <motion.path
              d="M9 14l4 4 6-7"
              ...
              animate={{ pathLength: pullProgress, opacity: pullProgress }}
            />
          </svg>
      ...
      {reviews.map((review, index) => (
        ...
        >
          {shouldPreload(index) ? (
            ...
            <div className="h-full w-full bg-void" aria-hidden="true" />
          )}

          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-void via-void/60 to-transparent p-5 pb-24">
            <VideoInfo
            ...
            <div className="mb-3 mt-4 flex items-center gap-4">
              ...
              <Link
                ...
                className="tap-48 flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-2 text-sm font-bold text-white/90 backdrop-blur-md transition-colors hover:bg-white/20 hover:text-white"
              >
            ...
            {!revealed.has(review.id) ? (
              ...
              <div className="relative mt-4 max-h-[60vh] overflow-auto rounded-3xl border border-white/10 bg-void/60 p-4 backdrop-blur-2xl no-scrollbar">
                {onPlayAgain && (
                  <button
                    onClick={() => onPlayAgain(review.id)}
                    aria-label="Close and keep scrolling"
                    className="absolute right-2 top-2 z-10 tap-48 rounded-full bg-white/10 p-2 text-white/70 transition-colors hover:bg-white/20 hover:text-white"
                  >
                  ...
                  if (!data) {
                    return (
                      <div className="text-center">
                        ...
                        <p className="font-heading text-6xl font-black tracking-tighter gradient-text">
                          {review.rating}
      ...
      {isLoadingMore && (
        <div className="flex h-32 items-center justify-center gap-3 px-5">
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        disabled={!selected}
        className={[
          "w-full min-h-14 rounded-full py-3.5 font-bold text-white shadow-lg transition-all",
          selected
            ? "bg-gradient-to-r from-primary-500 to-accent-pink shadow-glow hover:shadow-glow-lg hover:brightness-105"
            : "cursor-not-allowed bg-white/10 text-white/40 shadow-none ring-1 ring-white/10",
        ].join(" ")}
      >
        {selected ? "Lock it in" : "Pick a rating first"}
      </motion.button>
    </motion.div>
  );
}
