...
function isAllowedSegment(segment: string): boolean {
  return ALLOWED_PREFIXES.some((prefix) => {
    if (prefix === "dailyDrop") {
      // dailyDrop* matches the exact prefix or anything starting with it.
      return segment === prefix || segment.startsWith(prefix);
    }
    // feed.* and challenges.* match the exact prefix or prefix + ".".
    return segment === prefix || segment.startsWith(`${prefix}.`);
  });
}

function isDeniedSegment(segment: string): boolean {
  return DENIED_PREFIXES.some((prefix) => segment === prefix || segment.startsWith(prefix));
}
...
const idbStorage = {
  getItem: async (key: string): Promise<string | undefined> => {
    try {
      ...
      console.error("Failed to read persisted query cache from IndexedDB", err);
      return undefined;
    }
  },
  setItem: async (key: string, value: string): Promise<void> => {
    try {
      ...
      console.error("Failed to write persisted query cache to IndexedDB", err);
    }
  },
  removeItem: async (key: string): Promise<void> => {
    try {
      ...
      console.error("Failed to remove persisted query cache from IndexedDB", err);
    }
  },
};
